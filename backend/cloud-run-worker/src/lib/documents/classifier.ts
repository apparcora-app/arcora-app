...
const extractUtilityLateAmount = (text: string, primaryAmount?: number) => {
  ...
  const lateAmount = extractAmountByLabels(text, ['l.p.surcharge', 'late pay surcharge', 'penalty', 'fine', 'surcharge']);
  
  if (lateAmount !== undefined && lateAmount !== primaryAmount && !isLikelyYear(lateAmount)) {
    return lateAmount;
  }
...
const extractUtilityAccountNumber = (text: string) => {
  // Global filter: Remove lines that are known to contain "identifiers" instead of account numbers.
  // This prevents picking up SDO/XEN codes or Outstanding installment counts.
  const lines = text.split('\n');
  const sanitizedText = lines
    .filter((l) => !/outstanding|inst\.|\bxen\b|\bsdo\b|deferred/i.test(l))
    .join('\n');

  const fromLabels =
    extractField(sanitizedText, 'account id', [
      'account id/esn',
      'account no',
      'account number',
      'consumer no',
  ...
  if (
    sanitizedText.toLowerCase().includes('fesco') ||
    sanitizedText.toLowerCase().includes('supply company') ||
    sanitizedText.toLowerCase().includes('disco')
  ) {
    // Priority: 1. Consumer ID (10 digits starting from 1), 2. Reference Number (14 digits)
    const consId = sanitizedText.match(/\b(1\d{9})\b/);
    if (consId) return consId[1];

    const refNum = extractDISCOReferenceNumber(sanitizedText);
    if (refNum) return refNum;
  }

  if (sanitizedText.toLowerCase().includes('sui northern') || sanitizedText.toLowerCase().includes('sngpl')) {
    const sngpl = extractSNGPLAccount(sanitizedText);
    if (sngpl) return sngpl;
  }

  if (sanitizedText.toLowerCase().includes('pg&e') || sanitizedText.toLowerCase().includes('pacific gas')) {
    const pge = extractPGEAccount(sanitizedText);
    if (pge) return pge;
  }
  ...
  for (const regex of accountIdRegexes) {
    // Use sanitizedText to prevent picking up OUTSTANDING/XEN numbers via regex
    const match = sanitizedText.match(regex);
    if (match?.[1]) return match[1];
  }

  // Use sanitizedText for fallback long number search too
  const fallbackLongNumbers = sanitizedText.match(/\b\d{9,20}\b/g) ?? [];
...
const extractFescoReferenceNumber = (text: string): string | undefined => {
  ...
  // 4) Fuzzy fallback for common misreads (e.g., B24 instead of 18)
  const fuzzy = windowText.match(/\b[A-Z0-9]{2,3}\s+[A-Z0-9]{5}\s+(\d{6,8})\s+(\d{3,5})\s*R\b/i);
  if (fuzzy) {
    return `18 ${fuzzy[1]} ${fuzzy[2]} R`;
  }
...
const extractDISCOReferenceNumber = (text: string): string | undefined => {
  ...
  const m = segment.match(/\b(\d{14})\b/);
  if (m) return m[1];
  // Spaced: 01 12345 1234567
  const spaced = segment.match(/\b(\d{2})\s+(\d{5})\s+(\d{7})\b/);
  if (spaced) return `${spaced[1]}${spaced[2]}${spaced[3]}`;

  // Fuzzy spaced: handles misread characters at start (often XX)
  const fuzzy = segment.match(/\b[A-Z0-9]{2}\s+(\d{5})\s+(\d{7})\b/);
  if (fuzzy) return `18${fuzzy[1]}${fuzzy[2]}`;
...
export const classifyUploadedDocument = (text: string, fileName: string): ClassificationResult => {
  ...
  const isDiscoBill = section === 'bills' && providerName && ELECTRICITY_PROVIDERS.includes(providerName as any);
  ...
  // Specifically extract just the surcharge amount (the penalty only).
  // FESCO uses label "L.P.SURCHARGE" or "L.P. SURCHARGE" on the bill.
  // We search for it using last-occurrence to prefer the footer summary section.
  const surcharge =
    isFescoBill
  ...
  const extractedData: ExtractedDocumentData = {
    ...
    rawFields: {
      ...
      extractedLateSurcharge: surcharge?.toString() ?? '',
      extractedLateAmountPayable: lateAmountTotal?.toString() ?? '',
      billingMonth: utilityBillingMonth ?? '',
      currency,
      marketRegion,
    },
